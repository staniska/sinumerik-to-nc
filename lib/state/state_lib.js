...
export function lib(state) {
        ...
        if (!state.waitForSinumerikHighlightActivate) {
            state.waitForSinumerikHighlightActivate = atom.packages.onDidActivatePackage(pack => {
                if (pack.name === 'sinumerik-highlight') {
                    ...
                    setTimeout(() => {
                        get_programData(state, filePath)
    ...
    if (state.sinumerikHighlightView) {
        state.plane = state.sinumerikHighlightView
            .programmData[atom.workspace.getActiveTextEditor().getPath()]
    ...
    state.parse.find_else = (endif_row) => {
        let level = 0;
        let else_row = 0;
        let row = state.parse.row_num + 1
        while (!else_row && row < endif_row) {
...
const get_programData = (state, fileKey) => {
    state.fanuc_type = null
    if (state.sinumerikHighlightView) {
        if (state.sinumerikHighlightView.programmData && state.sinumerikHighlightView.programmData[fileKey]) {
            state.programData = state.sinumerikHighlightView.programmData[fileKey]
