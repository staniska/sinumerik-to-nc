...
export function lib(state) {
            ...
            state.waitForSinumerikHighlightActivate = atom.packages.onDidActivatePackage(pack => {
                if (pack.name === 'sinumerik-highlight') {
                    ...
                    setTimeout(() => {
                        ...
                        find_machine_subroutines(state, fileName)
                    }, 1000)
                }
            })
        }
    ...
    if (!state.subroutines) {
        ...
        find_local_subroutines(state, dirPath)
        find_machine_subroutines(state, fileName)
        atom.workspace.onDidChangeActiveTextEditor(() => {
            state.subroutines.clear()
            find_local_subroutines(state, dirPath)
            find_machine_subroutines(App.sinumerikToNcView.state, fileName)
        })
    }
...
const find_local_subroutines = (state, dirPath) => {
...
const find_machine_subroutines = (state, fileName) => {
    if (!state.programData) return
    console.log(state.programData)

    console.log(state.sinumerikHighlightView.machineData)
}
...
class CSubroutinesArray extends Array {
    push(element) {
        if (this.filter(el => el.name === element.name).length) {
            return null
        }
        Array.prototype.push.call(this, element)
    }
