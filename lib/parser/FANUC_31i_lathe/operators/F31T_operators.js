...
import {else_replace, endif_replace, if_else_replace} from "../../FANUC/operators/F_if_endif";
...
import {replace_rot} from "../../FANUC/operators/F_rot";
import {replace_pause} from "../../FANUC/operators/F_pause";
...
export function F31T_operator_replace(operator) {
    ...
    if (operator.type === 'else') {
        resp_merge(resp, else_replace(operator))
    }
    if (operator.type === 'endif') {
        resp_merge(resp, endif_replace(operator))
    }
    ...
    if (operator.type === 'pause') {
        resp_merge(resp, replace_pause(operator))
    }
    ...
    if (operator.type === 'rot') {
            resp_merge(resp, replace_rot(operator))
    }
