...
function rowParse(row, parser) {
    ...
    if (row.match(';')) {
        ...
        resp_rows.push(parser.comment.prefix + row_with_comment[1] + parser.comment.postfix)
    }
    if (!row.length) {
    ...
    try {
        split_row.forEach((operator) => {
            ...
            if (operator_block.data.additions.length) {
                ...
                operator_block.data.additions.forEach((addition, idx) => {
