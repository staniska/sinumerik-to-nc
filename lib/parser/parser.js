...
export function parse(row, index, parser, state) {
    ...
    if (row.length) {
        ...
        // resp_row.title = row
        row_data = rowPasre(row, index, parser, state)
        if (row_data.err) {
...
function rowPasre(row, index, parser, state) {
    ...
    if (row.match(';')) {
        ...
        resp_rows.push(parser.comment_prefix + row_with_comment[1])
    }
    if (!row.length) {
    ...
    try {
        split_row.forEach((operator) => {
            let operator_block = document.createElement('div')
            operator_block.className = 'stnc_operator-block'
            operator_block.data = parser.operatorParse(operator, row, index, state)
    ...
    return {list: resp_list, rows: resp_rows, err: err}
}
