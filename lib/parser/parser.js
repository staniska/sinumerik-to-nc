...
function rowParse(row, parser) {
    ...
    try {
        ...
        split_row.forEach((operator, idx) => {
            ...
            if (operator_block.data.additionsEnd.length) {
                additionsEnd.push(...operator_block.data.additionsEnd)
            }
            ...
            operator_block.data.errors.forEach(error => {
                const langSelector = document.querySelector('.stnc_header_lang_div').querySelector('select')
                const lang = langSelector.options[langSelector.selectedIndex].text
                let err_text = errors[lang][error]
        ...
        if (additionsEnd.length) {
            block.additionsEnd = create_element(["stnc_operator-block__received-additions"], block, '', 'ul')
            block.additionsEnd.data = []
            additionsEnd
                .forEach((addition, idx) => {
                        block.additionsEnd.data[idx] =
                            create_element(
                                ["stnc_operator-block__received-additions"],
                                block.additionsEnd,
                                addition,
            ...
            additionsEnd.forEach(addition => {
                    resp_rows.push(addition)
                }
            )
        }
