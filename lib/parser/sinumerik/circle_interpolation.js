...
export function check_circle(row, operator) {
    ...
    if (operator.match(/[IJK]=AC\(/)) {
        ...
        let prevPosition = state.prevPosition[center_axes[resp.center.name]]
        if (prevPosition.match(/[+-]/)) {
            prevPosition = `(${prevPosition})`
        }
        resp.center.increment = `${resp.center.value} - ${prevPosition}`
