    ...
    sinumerik_maths.forEach((math, idx) => {
        ...
        if (string.match(regExp)) {
            ...
            if (math === 'ATAN2') {
                ...
                //TODO Important!! Replace ATAN by ATAN[#]/[#]!!!!
                if (info.find(i => i.match('ATAN2'))) {
                    info.push('ATAN2(sinumerik) & ATAN(fanuc) are equal only for the first quarter of the unit circle')
                    info.push('check the values of the corresponding macro variables')
                }
            }
